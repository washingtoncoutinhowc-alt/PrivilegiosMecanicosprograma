...
const supabaseConfig = window.SPM_SUPABASE_CONFIG || {
  url: "",
  anonKey: ""
};
...
async function signOut() {
  if (supabaseRuntime) {
    try {
      ...
      // Mantem a saida local mesmo se a rede falhar.
    }
  }
...
async function signInWithGoogle() {
  ...
  if (!isSupabaseConfigured()) {
    message.textContent = "O login Google ainda precisa da URL e chave publica do Supabase.";
    return;
  }
  try {
...
async function loadSupabaseSession() {
  ...
  try {
    ...
    let authorized = null;
    const response = await supabaseRuntime
    ...
    if (!response.error && response.data) {
      authorized = {
        ...
        role: response.data.perfil,
        active: response.data.ativo
      };
    }

    if (!authorized) {
      authorized = (state.authUsers || []).find((user) => normalizeEmail(user.email) === email && user.active);
    }
    ...
    if (!authorized) {
      ...
      state.currentUser = null;
      saveState();
      document.querySelector("#authMessage").textContent = "Acesso nao autorizado. Peça ao administrador para cadastrar seu e-mail.";
      return;
    }
    state.currentUser = {
      name: authorized.name || sessionUser.user_metadata?.full_name || email,
      email,
    ...
    document.querySelector("#authMessage").textContent = "Nao foi possivel verificar o login Google.";
  }
}
